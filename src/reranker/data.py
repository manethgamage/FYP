...
class GroupedTrainDataset(Dataset):
    ...
    def create_one_example(self, qry_encoding: List[int], doc_encoding: List[int]):
        if isinstance(qry_encoding, list):
            qry_encoding = self.tok.decode(qry_encoding, skip_special_tokens=True)
        if isinstance(doc_encoding, list):
            doc_encoding = self.tok.decode(doc_encoding, skip_special_tokens=True)
        item = self.tok._encode_plus(
            qry_encoding,
            doc_encoding,
